...
export const StrokeAnimation: React.FC = () => {
  ...
  let fillOpacity = 1.0;
  if (phase == 0) {
    dashLength = interpolate(frame, phaseRange, [0, 100])
    dashSpaceLength = 100
    logoOpacity = interpolate(frame, phaseRange, [0, 1])
    whiteX = interpolate(frame, phaseRange, [40, 0], { easing: Easing.inOut(Easing.sin) })
    whiteY = 0
    greenX = interpolate(frame, phaseRange, [0, 40], { easing: Easing.inOut(Easing.sin) })
    greenY = 40
    fillOpacity = 0
  }
  if (phase == 1) {
    dashLength = interpolate(frame, phaseRange, [100, 200])
    dashSpaceLength = interpolate(frame, phaseRange, [100, 0])
    fillOpacity = interpolate(frame, phaseRange, [0, 1])
    whiteY = interpolate(frame, phaseRange, [0, 40], { easing: Easing.inOut(Easing.sin) })
    greenY = interpolate(frame, phaseRange, [40, 0], { easing: Easing.inOut(Easing.sin) })
  }
  if (phase == 3) {
    logoOpacity = interpolate(frame, phaseRange, [1, 0])
    dashLength = interpolate(frame, phaseRange, [50, 0])
    dashSpaceLength = interpolate(frame, phaseRange, [0, 100])
    fillOpacity = interpolate(frame, phaseRange, [1, 0])
  }
  if (phase == 4) {
    logoOpacity = 0
    dashLength = 0
  ...
  return <div style={{ backgroundColor: 'black', width: '100%', display: 'flex', alignItems: 'center' }}>
    <svg viewBox="0 0 158 158" width="158" height="158" style={{ marginRight: '20px' }}><defs>
    ...
    <svg width="353" height="74" fill="none" viewBox="-1 -1 354 75">
      <path
        stroke="#fff"
        style={{
          strokeDasharray: `${dashLength} ${dashSpaceLength}`,
